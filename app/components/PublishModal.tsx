...
export function PublishModal({
  ...
  const publishingTasks = [
    { text: "Validating app details...", icon: "🔍" },
    { text: "Signing manifest with Farcaster...", icon: "✍️" },
    { text: "Creating app manifest...", icon: "📋" },
    { text: "Publishing to Farcaster...", icon: "🚀" },
    { text: "Finalizing deployment...", icon: "✨" },
  ];
  ...
  const handlePublish = async () => {
    logger.log("handlePublish called with:", {
      projectId,
      projectUrl,
      isInMiniApp,
    });
    ...
    if (!projectId) {
      ...
      setError(
        "Project ID is missing. Please ensure your project is loaded correctly."
      );
      return;
    }
    ...
    if (!projectUrl) {
      ...
      setError(
        "Project URL is missing. Please ensure your project is deployed."
      );
      return;
    }
    ...
    if (!isInMiniApp) {
      setError(
        "Publishing requires running in a Farcaster miniapp to sign the manifest."
      );
      return;
    }
    ...
    try {
      ...
      try {
        ...
        if (error?.name === "RejectedByUser") {
          throw new Error(
            "You cancelled the signing request. Please try again to publish your app."
          );
        } else if (error?.name === "InvalidDomain") {
          throw new Error("Invalid domain. Please check your project URL.");
        } else {
          throw new Error(
            error?.message || "Failed to sign manifest with Farcaster"
          );
        }
      }
          ...
          {currentStep === "form" && (
              ...
              {!isAuthenticated && (
                <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4">
                  <p className="text-sm text-yellow-800 font-medium">
                    ⚠️ You need to be signed in to publish. Please authenticate
                    first.
                  </p>
                </div>
              )}
              ...
              {!authLoading && !isInMiniApp && (
                <div className="bg-orange-50 border border-orange-200 rounded-lg p-4">
                  <p className="text-sm text-orange-800 font-medium">
                    ⚠️ Publishing requires running inside a Farcaster miniapp to
                    sign your manifest.
                  </p>
                </div>
              )}
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-sm text-purple-900">
                      Fill in your app details and upload images below. When you
                      click Publish, we&apos;ll sign your manifest using your
                      Farcaster account and deploy everything automatically.
                    </p>
                  </div>
                </div>
              </div>
           ...
           {currentStep === "publishing" && (
             <div className="flex flex-col items-center justify-center py-12">
               <div className="animate-spin rounded-full h-16 w-16 border-t-2 border-b-2 border-black mb-6"></div>
               ...
               {currentTaskIndex === 2 && (
                 <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 max-w-md">
                   <div className="flex items-center gap-2 text-sm text-blue-800">
                     ...
                     <span>Please approve the signing request in Farcaster...</span>
                   </div>
                 </div>
               )}
             </div>
          ...
          {currentStep === "success" && (
            <div className="flex flex-col items-center justify-center py-8">
              ...
              <p className="text-gray-600 text-center mb-6 max-w-md">
                You did a great job! Your miniapp is now deployed and ready to
              ...
              {shareableLink && (
                <div className="w-full bg-gradient-to-r from-purple-50 to-pink-50 border border-purple-200 rounded-xl p-5 mb-4">
                  ...
                  <button
                    ...
                    className="w-full mb-3 flex items-center justify-center gap-2 px-4 py-3 bg-[#6A3CFF] text-white rounded-full hover:bg-[#5930D9] transition-colors cursor-pointer font-medium"
                  >
                    <Image
                      src="/farcaster.svg"
                      alt="Farcaster"
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <button
                      ...
                      className="p-3 bg-gray-100 text-gray-700 rounded-full hover:bg-gray-200 transition-colors cursor-pointer"
                      title="Copy link"
                    >
                      <svg
                        className="w-5 h-5"
                  ...
                  <p className="text-xs text-purple-700 mt-2">
                    Share this link anywhere - it will open as a miniapp in
                    Farcaster!
                  </p>
                </div>
              )}
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-blue-800">
                      Your app is now live and can be shared via the link above!
                      Note that apps don&apos;t appear in the Farcaster Miniapp
                      Store directly - your app needs to gain users and traction
                      before it can be featured there.
                    </p>
                    <p className="text-sm text-blue-800 mt-2">
                      <strong>Tip:</strong> Share your app link on Farcaster, X
                      , and with your community to get more users!
                    </p>
                  </div>
                </div>
              </div>
        ...
        <div className="flex items-center justify-between p-6 border-t border-gray-200 bg-gray-50">
          {currentStep === "form" && (
            <>
              <button
                onClick={handleClose}
                className="px-4 py-2 text-gray-700 hover:bg-gray-200 rounded-full font-medium transition-colors cursor-pointer"
              >
                Cancel
              </button>
              <button
                ...
                className={`px-6 py-2 bg-black text-white rounded-full font-medium transition-colors ${
                  isLoading ||
                  !isAuthenticated ||
                  !isInMiniApp ||
                  !appName.trim() ||
          ...
          {currentStep === "publishing" && (
            <div className="w-full flex justify-center">
              <span className="text-sm text-gray-600">
                Please approve the signing request...
              </span>
            </div>
          )}
          {currentStep === "success" && (
